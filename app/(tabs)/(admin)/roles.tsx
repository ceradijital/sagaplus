...
export default function RolesScreen() {
  ...
  useEffect(() => {
    if (!permissionsLoading && hasPermission('admin.roles.manage')) {
      fetchData();
    }
  ...
  const handleSaveRole = async () => {
    ...
    try {
      ...
      // Permission güncelleme işlemi - önce yenileri ekle, sonra gereksizleri sil
      const currentPermissions = rolePermissions
      ...
      if (toAdd.length > 0) {
        ...
        if (addError) {
          ...
          Alert.alert('Hata', 'Yeni yetkiler eklenemedi: ' + addError.message);
          return;
        }
      ...
      if (toRemove.length > 0) {
        ...
        if (removeError) {
          ...
          Alert.alert('Uyarı', 'Bazı eski yetkiler kaldırılamadı: ' + removeError.message);
        } else {
